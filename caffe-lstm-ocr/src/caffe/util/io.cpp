...
cv::Mat ReadImageToCVMat2(const string& filename,
  ...
  return ReadImageToCVMat2(filename, height, width, true);
}

cv::Mat ReadImageToCVMat2(const string& filename,
    const bool is_color) {
  return ReadImageToCVMat2(filename, 0, 0, is_color);
}

cv::Mat ReadImageToCVMat2(const string& filename) {
  return ReadImageToCVMat2(filename, 0, 0, true);
}
...
bool ReadImageToDatum2(const string& filename, const int label,
  ...
  cv::Mat cv_img = ReadImageToCVMat2(filename, height, width, is_color);
  if (cv_img.data) {
    if (encoding.size()) {
