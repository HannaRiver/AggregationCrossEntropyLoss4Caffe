...
void DataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
  ...
  *do_mirror = param_.mirror() && Rand(2);
  ...
  cv::Mat cv_resized_image, cv_noised_image, cv_distorted_image, cv_cropped_image;
  if (param_.has_resize_param()) {
  ...
  if (param_.has_noise_param()) {
    cv_distorted_image = ApplyDistort(cv_noised_image, param_.distort_param());
  } else {
    cv_distorted_image = cv_noised_image;
  }
  int img_height = cv_distorted_image.rows;
  int img_width = cv_distorted_image.cols;
  ...
  if ((crop_h > 0) && (crop_w > 0)) {
    ...
    cv_cropped_image = cv_distorted_image(roi);
  } else {
    cv_cropped_image = cv_distorted_image;
  }
