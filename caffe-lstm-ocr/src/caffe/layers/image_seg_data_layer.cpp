...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  cv::Mat cv_img = ReadImageToCVMat2(root_folder + lines_[lines_id_].first,
...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int img_row, img_col;
    cv_img_seg.push_back(ReadImageToCVMat2(root_folder + lines_[lines_id_].first,
	  new_height, new_width, is_color, &img_row, &img_col));

    // TODO(jay): implement resize in ReadImageToCVMat2
    ...
    if (label_type == ImageDataParameter_LabelType_PIXEL) {
      cv_img_seg.push_back(ReadImageToCVMat2(root_folder + lines_[lines_id_].second,
