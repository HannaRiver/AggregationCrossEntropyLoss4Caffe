...
template <typename Dtype>
void SpatialTransformerLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
    ...
    for(int i=0; i<6; ++i)
    {
        ...
        {
            set_value_to_constant<Dtype><<<CAFFE_GET_BLOCKS(num_threads), CAFFE_CUDA_NUM_THREADS>>>(num_threads, pre_defined_theta[i], 6, i, full_theta_data);
        ...
        else
        {
            ...
            copy_values<Dtype><<<CAFFE_GET_BLOCKS(num_threads), CAFFE_CUDA_NUM_THREADS>>>(num_threads, 6 - pre_defined_count, k, theta, 6, i, full_theta_data);
    ...
    for(int i = 0; i < N; ++i)
    {
        ...
        //caffe_gpu_gemm: C = alpha*A*B+beta*C
    }
